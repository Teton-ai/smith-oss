...
pub async fn get_telemetry_for_device(
...
) -> Result<Json<types::TelemetryPaginated>, StatusCode> {
    ...
    let mut telemetry = if let Some(starting_after) = pagination.starting_after {
        sqlx::query_as!(
            types::Telemetry,
            r#"(select
                id,
    ...
    .map_err(|err| {
        error!("Failed to get telemetry for device {err}");
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    // Sort telemetry by timestamp (most recent first).
    telemetry.sort_by(|a, b| b.timestamp.cmp(&a.timestamp));

    let filtered_telemetry: Vec<&types::Telemetry> = telemetry
        .iter()
        .filter(|t| t.service == Some("systemd1".to_string()))
        .collect();

    let first_id = filtered_telemetry
        .first()
        .map(|t| t.id.expect("error: failed to get first telemetry id"));

    let last_id = filtered_telemetry
        .last()
        .map(|t| t.id.expect("error: failed to get last telemetry id"));
    ...
    let next = if has_more_last_id {
        Some(format!(
            "https://{}/devices/{}/telemetry?starting_after={}&limit={}",
            host.0,
            device_id,
            last_id.unwrap(),
            limit
    ...
    let previous = if has_more_first_id {
        Some(format!(
            "https://{}/devices/{}/telemetry?ending_before={}&limit={}",
            host.0,
            device_id,
    ...
    let telemetry_paginated = types::TelemetryPaginated {
        telemetry,
        next,
        previous,
    };

    Ok(Json(telemetry_paginated))
}
